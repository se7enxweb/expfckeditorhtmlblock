        ...
        <td>
            <input 	{if eq($Nodes.item.class_name, 'R�pertoire')}
            		...
            		{else}
            			...
            			imgheight="{$Nodes.item.data_map.image.content.original.height}"            			data-imgurl={$Nodes.item.data_map.image.content.original.url|ezroot}            		{/if}
            		value="expobject://{$Nodes.item.node_id}"
            type="radio" name="lien" />
        </td>
        {if ne($Nodes.item.class_name, 'R�pertoire')}
